...
function buildRegions() {
  ...
  regions.set('home-chart', `      <p class="chart-intro">Myers diff implemented in Rust, compiled to WebAssembly. A 5,000 line file diffs in about ${bigApprox} ms, measured around the engine call including view assembly, so the result updates live on every keystroke.</p>
      <div class="chart">
        <div class="chart-bar">
  ...
  <p class="bench-note">The M10 page path column is what the live site actually does per keystroke since M10: compute call plus lazy row model plus one 60-row render window; the virtualized page never materializes every row. It is shown for honesty about what you experience on the site, but the headline ratio deliberately uses the full-assembly total, where both sides do the same work.</p>`);
